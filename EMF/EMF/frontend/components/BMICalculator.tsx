...
// Local native compute function
function executeBMICalculation(form: { height_cm: string, weight_kg: string, gender: string, activity_level: string }): BMIResponse {
  ...
  if (weight < 20 || weight > 400) throw new Error("Weight must be between 20 and 400 kg");

  const height_m = height / 100.0;
  const bmi_value = weight / (height_m * height_m);

  let category = "";
  let recs: string[] = [];
  if (bmi_value < 18.5) {
    category = "Underweight";
    recs = ["Focus on caloric surplus.", "Increase complex carbs and protein.", "Strength train to build muscle."];
  } else if (bmi_value < 25) {
    category = "Normal weight";
    recs = ["Maintain current activity.", "Eat a balanced diet.", "Focus on progressive overload."];
  } else if (bmi_value < 30) {
    category = "Overweight";
    recs = ["Slight caloric deficit recommended.", "Boost daily step count.", "Prioritize high protein meals."];
  } else {
    category = "Obesity";
    recs = ["Consult a professional trainer.", "Focus on joint-friendly cardio.", "Track daily calories closely."];
  }

  // Mifflin-St Jeor
  const age = 30; // default for estimation
  let bmr = (10 * weight) + (6.25 * height) - (5 * age);
  if (form.gender === "male") bmr += 5;
  else bmr -= 161;
  ...
  const tdee = Math.round(bmr * (multipliers[form.activity_level] || 1.2));
...
export default function BMICalculator() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = executeBMICalculation(form);
      setResult(res);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Calculation failed");
    } finally {
